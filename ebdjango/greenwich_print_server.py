...
try:
    ...
    if greenwich_print_server_secret_key == '':
        print("Trying to read the GREENWICH_PRINT_SERVER_SECRET_KEY environment variable failed.")
...
try:
    def fetch_print_jobs():
        try:
            ...
            params = {'secret_key': greenwich_print_server_secret_key}
            response = requests.get('https://{}/api/printjobs/'.format(domain), params=params) #changed to https; not sure if this will work
...
try:
    def print_to_receipt_printer(jobs):
        ...
        printed_jobs = []
        PRINTER_IP = '192.168.105.50' #'10.186.3.27' # changed to greenwich static ip
...
try:
    def return_print_job_results(printed_jobs):
        try:
            ...
            response = requests.post('https://{}/api/printjobs/'.format(domain), params=params) # changed to https; not sure if this will work
            print("Response: ", response)
            response.raise_for_status()  # Raises an error for bad responses
            #print_jobs = response.json()  # Assuming the response is JSON
            return response
